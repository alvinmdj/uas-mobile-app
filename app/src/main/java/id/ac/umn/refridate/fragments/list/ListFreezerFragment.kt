...
import kotlinx.android.synthetic.main.fragment_freezer_list.view.*
import java.lang.Exception

class ListFreezerFragment : Fragment() {

    private lateinit var database: DatabaseReference
    private lateinit var itemList: List<ItemList>

    private lateinit var recyclerView: RecyclerView
    private lateinit var adapter: ListFreezerAdapter
    private lateinit var layoutManager: RecyclerView.LayoutManager

    override fun onCreateView(inflater: LayoutInflater, container: ViewGroup?,
                              savedInstanceState: Bundle?): View? {
        val view = inflater.inflate(R.layout.fragment_freezer_list, container, false)

        database = FirebaseDatabase.getInstance().getReference("ItemList")
        val userId = FirebaseAuth.getInstance().currentUser?.uid.toString()
        val itemListQuery = database.child(userId).orderByChild("place").equalTo("Freezer")

        itemList = ArrayList()
        recyclerView = view.recyclerViewFreezer
        layoutManager = LinearLayoutManager(requireContext())
        recyclerView.layoutManager = LinearLayoutManager(requireContext())
        adapter = ListFreezerAdapter()
        ...
        itemListQuery.addValueEventListener(object : ValueEventListener {
            override fun onDataChange(snapshot: DataSnapshot) {
                try {
                    ...
                    for (itemSnapshot in snapshot.children) {
                        ...
                        Log.d(TAG, "onDataChange (KEY) : " + itemSnapshot.key!!)
                    }
            ...
            override fun onCancelled(error: DatabaseError) {
                Log.w(TAG, "itemList:onCancelled", error.toException())
                Toast.makeText(context, "Gagal memuat data.", Toast.LENGTH_SHORT).show()
            }
        })

        view.floatingActionButton.setOnClickListener{
            findNavController().navigate(R.id.action_listFreezerFragment_to_addFreezerFragment)
        }
